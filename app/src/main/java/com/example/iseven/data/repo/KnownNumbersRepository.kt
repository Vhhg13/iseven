...
class KnownNumbersRepository(val dataSource: KnownNumbersDataSource) {
    ...
    suspend fun submit(evenness: Evenness) = dataSource.submit(KnownNumberEntity(evenness.number, evenness.isEven, evenness.ad))
    ...
    suspend fun remove(n: Int) = dataSource.remove(n)
}